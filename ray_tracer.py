...
def main():
    ...
    ray_colors = []
    counter = 0
    for i, ray in enumerate(pixel_rays):
        # Directly compute the color for each ray using trace_ray
        ray_colors.append(trace_ray(ray, 0, max_depth, surfaces, materials, lights, camera, scene_settings))
        counter += 1
        if counter % 1000 == 0:
