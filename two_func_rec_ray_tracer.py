...
def generate_rays(camera, screen_center, pixel_width, pixel_height, image_width, image_height):
    ...
    samples_per_pixel = 4
    sample_offsets = np.linspace(-0.5, 0.5, int(np.sqrt(samples_per_pixel)))
        ...
        for j in range(image_width):
            ...
            for dx in sample_offsets:
                for dy in sample_offsets:
                    # Compute the pixel position on the screen with small offset for sampling
                    pixel_center = (screen_origin +
                                    camera.camera_right_vector * ((image_width - j - 0.5) * pixel_width + dx * pixel_width) +
                                    camera.up_vector * ((i + 0.5) * pixel_height + dy * pixel_height))

                    # Compute the ray direction
                    ray_direction = vector_utils.normalize_vector(pixel_center - camera.position)
                    rays.append(Ray(camera.position, ray_direction))
            row_rays.append(rays)
        pixel_rays.append(row_rays)
